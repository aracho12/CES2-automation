...
def generate_ces2_scripts(
    *,
    ...
    L("  # + velocity reinit before NVT.  minimize zeros all velocities, so the Nose-Hoover")
    L("  # thermostat has nothing to rescale and grid forces drive explosive dynamics.")
    L("  if [ $qmmmstep -eq 0 ] && ! [ -e \"$LAMMPSRESTART\" ]; then")
    L("    sed -i \"s/#MINIMIZE_STEP/minimize 1.0e-4 1.0e-6 1000 10000/\" in.lammps.equil")
