...
def generate_ces2_scripts(
    *,
    ...
    slab_elements       : type_labels of QM slab atoms (for QMLMPTYPE)
    type_id_to_element  : LAMMPS type integer → element symbol (for direct atoms= generation)
    """
    ...
    # Direct atoms=(El1 El2 ...) — one entry per LAMMPS type, 0-indexed (atoms[type-1]).
    # This replaces the fragile runtime mass-matching loop (which fails for elements
    # absent from the hardcoded ATOMMASS table, e.g. Cs, Ba, lanthanides).
    if type_id_to_element:
        n_types = max(type_id_to_element)
        elems = " ".join(type_id_to_element.get(t, "X") for t in range(1, n_types + 1))
        L(f"atoms=({elems}) # auto-generated by cesbuild: index = LAMMPS_type - 1")
    else:
        # Fallback: keep legacy mass-matching arrays (incomplete for rare elements)
        L("ATOMNAME=(H Li Be B C N O F Na Mg Al Si P S Cl K Ca Sc Ti V Cr Mn Fe Co Ni Cu Zn Ga Ge As Se Br Ag I Xe Cs Ba W Re Os Ir Pt Au)")
        L("# Truncate to 1 decimal place")
        L("ATOMMASS=(1.0 6.9 9.0 10.8 12.0 14.0 15.9 18.9 22.9 24.3 26.9 28.0 30.9 32.0 35.4 39.0 40.0 44.9 47.8 50.9 51.9 54.9 55.8 58.9 58.6 63.5 65.3 69.7 72.6 74.9 78.9 79.9 107.8 126.9 131.3 132.9 137.3 183.8 186.2 190.2 192.2 195.0 196.9)")
    ...
    if type_id_to_element:
        # atoms=(...) already set in header — just echo it for logging
        L('    echo "### Atom species (auto-generated by cesbuild): ${atoms[@]}"$\'\\n\'')
    else:
        # Legacy fallback: derive atoms=(...) at runtime from data.file masses
    ...
    L("    # Remove read_dump when restart provides coords (step > 0, or step 0 with restart)")
    L("    if [ $qmmmstep -gt 0 ] || [ -e \"$LAMMPSRESTART\" ]; then")
    L("      sed -i '/read_dump/d' in.lammps")
    L("    fi")
    ...
    L("  sed -i '/dump_modify.*dispTraj/d' in.lammps.equil")
    L("  sed -i \"s/.*#CUBEPOSITION.*/&\\ngrid\\t\\t ${cubeioequil} /\" in.lammps.equil")
    L("  # mm_0 without restart: classically pre-equilibrated structure needs DFT-CES minimize")
    L("  # + SHAKE geometry correction + velocity reinit before NVT.")
    L("  #")
    L("  # Why three steps instead of just velocity reinit:")
    L("  #   1. LAMMPS minimize cannot run with fix shake active (hard error).")
    L("  #   2. Without SHAKE, minimize distorts O-H bonds (avg delta ~0.1 A).")
    L("  #   3. When NVT+SHAKE starts, the bond correction impulse is Dv = Dr/dt.")
    L("  #      At dt=1 fs and Dr=0.1 A this is ~1000 m/s per H — enough to spike")
    L("  #      temperature by hundreds of K even after a velocity reinit.")
    L("  #")
    L("  # Fix: after minimize, run ONE NVE+SHAKE step with v=0.  SHAKE corrects")
    L("  # bond POSITIONS; the resulting velocities are immediately discarded by")
    L("  # the subsequent 'velocity create'.  Production SHAKE then sees already-")
    L("  # correct geometry and applies only negligible corrections each step.")
    L("  #")
    L("  # min_modify dmax 0.05 limits per-step displacement to reduce distortion.")
    L("  if [ $qmmmstep -eq 0 ] && ! [ -e \"$LAMMPSRESTART\" ]; then")
    L("    # (1) Replace minimize marker")
    L("    #     etol=0: disable energy-tolerance stopping — huge EgridRep initial energy")
    L("    #     makes |dE/E| trivially small so etol fires after 1 step, leaving forces")
    L("    #     at ~80000 kcal/mol/A.  Force tolerance (ftol=100) is the real criterion.")
    L("    #     dmax=0.05: small per-step displacement limits bond distortion.")
    L("    sed -i 's/#MINIMIZE_STEP/min_modify dmax 0.05\\nminimize 0.0 100.0 5000 50000/' in.lammps.equil")
    L("    # (2) Replace SHAKE correction marker — fix bond geometry before velocity reinit")
    L("    #     Extract SHAKE spec from the shakeH fix line (tol iter maxiter a N b N)")
    L("    shake_spec=$(awk '/fix[[:space:]]+shakeH[[:space:]]+SOLVENT[[:space:]]+shake/{")
    L("      for(i=4;i<=NF;i++) printf \"%s \", $i; print \"\"; exit}' in.lammps.equil | xargs)")
    L("    awk -v sp=\"$shake_spec\" '")
    L("      /#SHAKE_CORRECT_STEP/ {")
    L("        print \"velocity SOLVENT set 0.0 0.0 0.0\"")
    L("        print \"fix shakeCorr SOLVENT \" sp")
    L("        print \"fix nveCorr   SOLVENT nve/limit 0.05\"")
    L("        print \"run 1\"")
    L("        print \"unfix nveCorr\"")
    L("        print \"unfix shakeCorr\"")
    L("        next}")
    L("      {print}' in.lammps.equil > in.lammps.equil.tmp && mv in.lammps.equil.tmp in.lammps.equil")
    L("    # (3) Velocity reinit at target temperature — overwrites SHAKE-induced velocities")
    L("    nvt_T=$(grep 'fix.*nvt SOLVENT nvt' in.lammps.equil | awk '{print $6}')")
    L("    sed -i \"s/#VELOCITY_STEP/velocity SOLVENT create ${nvt_T} 12345 mom yes rot yes dist gaussian/\" in.lammps.equil")
    L("  else")
    L("    sed -i '/#MINIMIZE_STEP/d' in.lammps.equil")
    L("    sed -i '/#SHAKE_CORRECT_STEP/d' in.lammps.equil")
    L("    sed -i '/#VELOCITY_STEP/d' in.lammps.equil")
    L("  fi")
    L("  mpirun -np $NP $LAMMPS -in in.lammps.equil > lammps.equil.out")
    L("  equil_rc=$?")
    L("  if [ $equil_rc -ne 0 ]; then")
    L('    echo "### ERROR: LAMMPS equilibration failed with exit code $equil_rc at QMMM step $qmmmstep"')
    L('    echo "### Check lammps.equil.out and stderr for details"')
    L("    cp lammps.equil.out in.lammps.equil mm_$qmmmstep/ 2>/dev/null")
    L("    exit 1")
    L("  fi")
    L("  # Verify restart file was produced by equilibration")
    L("  if ! ls *.restart 1>/dev/null 2>&1; then")
    L('    echo "### ERROR: LAMMPS equilibration produced no restart files at QMMM step $qmmmstep"')
    L("    cp lammps.equil.out in.lammps.equil mm_$qmmmstep/ 2>/dev/null")
    L("    exit 1")
    L("  fi")
    L("    else")
    L('      echo "### Skipping LAMMPS equilibration (skip_equil=1)"')
    L("      skipequil=0  # reset so next steps run normally")
    L("    fi")
    L()
    L("  # RUN LAMMPS averaging step")
    L("  # If in.lammps was not prepared by equil (e.g. skip_equil), prepare it now")
    L("  if [ ! -e in.lammps ] || [ ! -e in.lammps.equil ]; then")
    L("    cp $LAMMPSIN in.lammps")
    L("  fi")
    L("  # Always remove read_dump before averaging: positions come from the equil restart file.")
    L("  # Keeping read_dump would overwrite equilibrated coords with pre-equil dump → wrong image flags.")
    L("  sed -i '/read_dump/d' in.lammps")
    L("  LAMMPSRESTART=`ls -lrt *.restart 2>/dev/null | tail -n 1 | awk '{print $9}'`")
    L('  if [ -z "$LAMMPSRESTART" ]; then')
    L('    echo "### ERROR: No restart file found for averaging step at QMMM step $qmmmstep"')
    L('    echo "### The equilibration step likely failed to produce restart files"')
    L("    exit 1")
    L("  fi")
    L("  LAMMPSRESTARTtime=`echo $LAMMPSRESTART | cut -f2 -d '.'`")
    L('  if [ -z "$LAMMPSRESTARTtime" ]; then')
    L('    echo "### ERROR: Could not parse timestep from restart file: $LAMMPSRESTART"')
    L("    exit 1")
    L("  fi")
    L("  sed -i \"s/.*reset_timestep.*/reset_timestep ${LAMMPSRESTARTtime}/\" in.lammps")
    ...
    L("  mpirun -np 1 $LAMMPS -r $LAMMPSRESTART data.md")
    L("  if [ $? -ne 0 ]; then")
    L('    echo "### ERROR: LAMMPS restart-to-data conversion failed at QMMM step $qmmmstep"')
    L("    exit 1")
    L("  fi")
    ...
    L("  sed -i '/dump_modify.*dispTraj/d' in.lammps")
    ...
    L("  avg_rc=$?")
    L("  if [ $avg_rc -ne 0 ]; then")
    L('    echo "### ERROR: LAMMPS averaging failed with exit code $avg_rc at QMMM step $qmmmstep"')
    L('    echo "### Check lammps.average.out and stderr for details"')
    L("    cp lammps.average.out in.lammps mm_$qmmmstep/ 2>/dev/null")
    L("    exit 1")
    L("  fi")
    L("  LAMMPSRESTART=`ls -lrt *.restart 2>/dev/null | tail -n 1 | awk '{print $9}'`")
    L('  if [ -z "$LAMMPSRESTART" ]; then')
    L('    echo "### ERROR: LAMMPS averaging produced no restart files at QMMM step $qmmmstep"')
    L("    cp lammps.average.out in.lammps mm_$qmmmstep/ 2>/dev/null")
    L("    exit 1")
    L("  fi")
    L("  rm -f log.lammps")
    ...
    if use_pbs:
        module_lines_sub = str(pbs_cfg.get("module_lines",
                                            sc_cfg.get("module_lines", "")))
    else:
    ...
    # ── Optional MM pre-equilibration (md_relax) ──────────────────────────
    ...
    if relax_enabled:
        ...
        SL(f'NP={np_procs}')
        SL()
        SL('echo "==== Relax Part 1: Soft pushoff + CG minimize ===="')
        SL("mpirun -np $NP $LMP -in in.relax_min || true")
        SL('if [ ! -f minimized.dump ]; then')
        SL('    echo "ERROR: Relax Part 1 failed — minimized.dump not found."; exit 1')
        SL("fi")
        SL('echo "    -> minimized.dump OK"')
        SL()
        SL('echo "==== Relax Part 2: NVT heating ===="')
        SL("mpirun -np $NP $LMP -in in.relax_heat || true")
        SL('if [ ! -f heated.dump ]; then')
        SL('    echo "ERROR: Relax Part 2 failed — heated.dump not found."; exit 1')
        SL("fi")
        SL('echo "    -> heated.dump OK"')
        SL()
        SL('echo "==== Relax Part 3: NVT equilibration ===="')
        SL("mpirun -np $NP $LMP -in in.relax_equil || true")
        SL('if [ ! -f equilibrated.dump ]; then')
        SL('    echo "ERROR: Relax Part 3 failed — equilibrated.dump not found."; exit 1')
        SL("fi")
        SL('echo "    -> equilibrated.dump OK"')
        SL()
        SL('echo "==== Relax done — starting QM/MM ===="')
