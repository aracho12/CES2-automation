...
def generate_ces2_scripts(
    *,
    ...
    equil_timestep = float(sc_cfg.get("equil_timestep_fs", 1.0))
    ...
    L("\t\telse ")
    L("\t\t\tconv=`echo ${MMpartialCharge[$i]} ${temprep[$i]} | awk '{printf(\"%f\",($1==0)?0:1/$1*$2*2)}'`")
    L("\t\t\techo $conv")
    ...
    L("    ## Preparation of QM")
    L("    # Resume safety: a resubmitted process loses the carried-over")
    L("    # $LAMMPSRESTART (it resets to its init value 'ini.restart'), so for")
    L("    # step > 0 recover the latest work-dir restart to continue from the")
    L("    # previous step instead of restarting from the initial structure")
    L("    # (ini.restart/data.file). In a continuous run this is a no-op because")
    L("    # the variable already holds the previous step's averaging restart.")
    L("    if [ $qmmmstep -gt 0 ]; then")
    L("      _latest_restart=`ls -lrt *.restart 2>/dev/null | tail -n 1 | awk '{print $9}'`")
    L("      # Robust resume: if the work-dir restarts were purged (e.g. scratch")
    L("      # cleanup or rsync to a fresh dir), recover the latest restart that")
    L("      # was archived under the previous step's mm_$((qmmmstep-1))/ directory")
    L("      # (copied there at the end of every MM step). Without this fallback the")
    L("      # MD trajectory would silently reset to the initial structure.")
    L('      if [ -z "$_latest_restart" ]; then')
    L('        _prev_mm="mm_$((qmmmstep-1))"')
    L("        _prev_restart=`ls -lrt ${_prev_mm}/*.restart 2>/dev/null | tail -n 1 | awk '{print $9}'`")
    L('        if [ -n "$_prev_restart" ]; then')
    L('          cp "$_prev_restart" ./')
    L('          _latest_restart=`basename "$_prev_restart"`')
    L('          echo "### Resume: no work-dir restart; recovered $_latest_restart from ${_prev_mm}/"')
    L("        fi")
    L("      fi")
    L('      [ -n "$_latest_restart" ] && LAMMPSRESTART="$_latest_restart"')
    L("    fi")
    L('    if [ -e "$LAMMPSRESTART" ]; then')
