...
def generate_qe_input(
    *,
    export_dir: Path,
    slab_cell: "np.ndarray",      # shape (3,3) — slab supercell cell matrix (BEFORE electrolyte z expansion)
    ...
    slab_cell    : 3×3 cell matrix of the slab *supercell* (before adding electrolyte z).
    ...
    # ── Unit cell for QE (= slab supercell / rep) ─────────────────────────
    # For an orthogonal cell the relevant parameters are diagonal elements.
    Lx_sc = float(slab_cell[0, 0])
    Ly_sc = float(slab_cell[1, 1])
    Lz_sc = float(slab_cell[2, 2])
    a_qe  = Lx_sc / rep[0]
    b_qe  = Ly_sc / rep[1]
    c_qe  = Lz_sc / rep[2]
    ...
    PW("!")
    PW(f"! Supercell : {rep[0]}×{rep[1]}×{rep[2]}  →  QE unit cell {a_qe:.4f} × {b_qe:.4f} × {c_qe:.4f} Å")
