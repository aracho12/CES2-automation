...
def generate_ces2_scripts(
    *,
    ...
    initial_qm     = int(sc_cfg.get("initial_qm",      1))
    ...
    L("ATOMNAME=(H Li Be B C N O F Na Mg Al Si P S Cl K Ca Sc Ti V Cr Mn Fe Co Ni Cu Zn Ga Ge As Se Br Ag I W Au)")
    L("# Truncate to 1 decimal place")
    L("ATOMMASS=(1.0 6.9 9.0 10.8 12.0 14.0 15.9 18.9 22.9 24.3 26.9 28.0 30.9 32.0 35.4 39.0 40.0 44.9 47.8 50.9 51.9 54.9 55.8 58.9 58.6 63.5 65.3 69.7 72.6 74.9 78.9 79.9 107.8 126.9 183.8 196.9)")
    ...
    L("    mpirun -np 1 $LAMMPS -r $LAMMPSRESTART data.temp")
    L("  else")
    L('    if [ -e "$LAMMPSDATA" ]; then')
    L("      cp $LAMMPSDATA data.temp # If there is no restart file")
    L("    else")
    ...
    L("    natoms_qm=`grep nat $QMIN | sed 's/,//' | awk '{print $3}'`")
    ...
    L("    Box=(`awk '{if(NF==4 && ($3==\"xlo\"||$3==\"ylo\"||$3==\"zlo\")) print $0}' data.temp`)")
    L("    cells=( `printf \"%f\" ${Box[1]}` `printf \"%f\" ${Box[5]}` `printf \"%f\" ${Box[9]}` ) # Cell box sizes 0:x 1:y 2:z")
    L('    echo "### Parsed lammps cell box size : ${cells[@]}"$\'\\n\'')
    L("    Masses=(`awk 'BEGIN{tag=0;cnt=0}{if($2==\"atom\"&&$3==\"types\") natoms=$1; if($1==\"Masses\") tag=1; if(tag==1 && NF==2 && cnt<natoms) {print $0; cnt++}}' data.temp`)")
    ...
    L("  awk -v nsolvatom=$NSOLVATOM '{if(NF==10 && $1>nsolvatom) print $0}' data.temp > data.solute.temp")
