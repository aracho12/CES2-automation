...
def set_ylim_top_margin(ax, margin=0.10):
    """Set y-axis limits so there is `margin` (fraction) of padding above the
    highest data point.  The bottom limit is kept at the natural data minimum.

    Args:
        ax     : Matplotlib Axes object
        margin : Fractional headroom above y_max (default 0.10 = 10 %)

    Example:
        ps.set_ylim_top_margin(ax)          # 10 % top padding
        ps.set_ylim_top_margin(ax, 0.15)    # 15 % top padding
    """
    ax.autoscale_view()                      # make sure limits reflect all artists
    y_lo, y_hi = ax.get_ylim()
    span = y_hi - y_lo
    ax.set_ylim(y_lo, y_hi + span * margin)
